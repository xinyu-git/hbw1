...
dialog={
	...
	closeDiv:function(){
		$("#alertInfo").stop(true,true).animate({
			...
			$("body > div:eq(0)").removeClass("filterBlur");
		});
	},
	//
	maskLayer:function(){
		...
		$("body > div:eq(0)").addClass("filterBlur");
	...
	alertLog:function(logTitle,func){//func：为“登录”按钮动作处理函数
			...
			+"  </div></li>"
			+"  <li class='yzM'><label for='yzM'>验证码：</label><input type='text' validate='true' name='yzM' id='yzM' maxlength='4' msg='请输入验证码'><img src='http://hd.wot.kongzhong.com/verifyCode' id='verifyImg' class='verifyImg' onclick='Util.verifyImg()'></li>"
			+"  <li class='tsli'><label for=''></label><span  id='loginTips'></span></li>"
			+" </ul></form>"
			+" <div class='infoBtn'><a class='click-btn' href='javascript:;' onclick='Util.login(Util.LOGIN_TYPE_LOGIN);'>登 录</a></div>"
			+"</div>");
...
/*function refreshRem(){
	...
	if(width>640){
		width=640;
		$("body,#wrap").css("max-width","640px");
	};
	if(width<320) width=320;
	var rem=Math.floor(width/640*100);
